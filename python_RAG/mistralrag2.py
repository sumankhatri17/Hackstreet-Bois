...
per_file_objects: Dict[str, List[Dict[str, Any]]] = {}
folder_path = "python_RAG/data"   # directory containing .jsonl/.ndjson
chunk_size = 2048
chunk_overlap = 200
...
if not folder.exists() or not folder.is_dir():
    print(f"Warning: folder_path {folder} does not exist or is not a directory. No documents loaded.")
else:
            ...
            if lower.endswith((".jsonl", ".ndjson")):
                ...
                with file_path.open("r", encoding="utf-8") as f:
                    for line_no, line in enumerate(f, start=1):
                        ...
                        # If the line is a list, expand; if dict, keep
                        if isinstance(obj, list):
                            # wrap each list element as a doc if primitive, or use dict if it's already a dict
                            for item in obj:
                                if isinstance(item, dict):
                                    ...
                                    wrapped = {"id": str(uuid.uuid4()), "file": filename, "text": str(item)}
                                    objs.append(wrapped)
                                    all_objects.append(wrapped)
                        elif isinstance(obj, dict):
                            obj.setdefault("id", str(uuid.uuid4()))
                            obj.setdefault("file", filename)
                            objs.append(obj)
                            all_objects.append(obj)
                        else:
                            wrapped = {"id": str(uuid.uuid4()), "file": filename, "text": str(obj)}
                            objs.append(wrapped)
                            all_objects.append(wrapped)
                ...
                # If the JSON file is a list of items, handle each element
                if isinstance(jo, list):
                    # If list contains chapter-like dicts, combine questions per chapter
                    for item in jo:
                        if isinstance(item, dict) and "questions" in item and isinstance(item["questions"], list):
                            ...
                            joined_text = "\n\n".join([f"Q{i+1}. {q}" for i, q in enumerate(questions)]) if questions else json.dumps(item, ensure_ascii=False)
                            doc = {
                                "id": str(uuid.uuid4()),
                                "file": filename,
                            ...
                            doc.setdefault("file", filename)
                            objs.append(doc)
                            all_objects.append(doc)
                        elif isinstance(item, dict):
                            # fallback: stringify dict into one document
                            doc = {"id": str(uuid.uuid4()), "file": filename, "text": json.dumps(item, ensure_ascii=False)}
                            objs.append(doc)
                            all_objects.append(doc)
                        else:
                            wrapped = {"id": str(uuid.uuid4()), "file": filename, "text": str(item)}
                    ...
                    # If dict has "questions" array, combine into single text for the chapter
                    if "questions" in jo and isinstance(jo.get("questions"), list):
                        chapter_title = jo.get("chapter")
                        # keep only string questions; if question items are objects, attempt to extract string field
                        questions: List[str] = []
                        for q in jo["questions"]:
                            if isinstance(q, str):
                                ...
                                q_text = q.get("text") or q.get("question") or q.get("q")
                                if isinstance(q_text, str):
                                    questions.append(q_text)
                                else:
                                    # fallback to JSON string of the item
                                    questions.append(json.dumps(q, ensure_ascii=False))
                            else:
                        ...
                        if questions:
                            ...
                            joined_text = json.dumps(jo, ensure_ascii=False)
                        ...
                        # If dict already looks like a document (has text/content), keep it
                        if "text" in jo or "content" in jo:
                            doc = jo.copy()
                            doc.setdefault("id", str(uuid.uuid4()))
                            doc.setdefault("file", filename)
                            objs.append(doc)
                            ...
                            # fallback: stringify entire dict into one document
                            doc = {"id": str(uuid.uuid4()), "file": filename, "text": json.dumps(jo, ensure_ascii=False)}
                            objs.append(doc)
                            all_objects.append(doc)

                else:
                    # Primitive -> wrap into one document
                    doc = {"id": str(uuid.uuid4()), "file": filename, "text": str(jo)}
                    objs.append(doc)
                    all_objects.append(doc)
...
# Summary
print(f"Loaded {len(per_file_objects)} files and {len(all_objects)} document objects.")
for fname, objs in per_file_objects.items():
    print(f"  {fname}: {len(objs)} objects")
...
while True:
    ...
    prompt = (
        ...
        "And For English QUestion Generation Strictly Follow those rules: 1) Provide Complete Questions With Context 2) If you are providing with Passage question make sure to include the context or the passage. 3) Make sure to provide complete and sensical questions\n"
        f"{context_text}\n\n"
        f"Question: {question}\n\nAnswer:"
    )
